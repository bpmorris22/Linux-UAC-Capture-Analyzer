...
| **Persistence** | 25+ mechanisms | Config inventory + scored command lines: cron (+ `cron.{hourly,daily,weekly,monthly}` & anacron), systemd units/timers + `Environment=LD_PRELOAD` / socket `ListenStream` / generators, authorized_keys (forced-command & `environment=`), rc.local (+RHEL) & init.d, ld.so.preload & `ld.so.conf.d`, motd & APT hooks, modprobe, XDG autostart, shell startup files, **PAM module dirs**, and **RAT/C2 spots** (system profile.d, **SSH-login rc & sshrc**, **network-event hooks**, module-autoload, udev RUN, binfmt, at jobs) - every command line also gets a **C2 tradecraft lens** (reverse-shell/backpipe/encoded-loader incl. php/ruby/gawk & `/dev/udp`, beacon cadence, known-implant names, hardcoded callback IPs / IPv6 / domains) |
